...
- [ ] **0.2 确认前置能力调用边界**
  ...
  - 验证：`pnpm test -- tests/terminal-process.spec.ts`；`pnpm exec tsc --noEmit`。

## 阶段 1：Workspace 配置和终端启动

- [ ] **1.1 实现 Workspace 配置文件读写和校验**
  - 状态：TODO
  ...
  - 验证：配置解析/路径守卫定向测试；`pnpm exec tsc --noEmit`。
...
- [ ] **1.2 接入已有 PTY 启动器**
  ...
  - 验证：`pnpm test -- tests/terminal-process.spec.ts` 和新增启动调用测试；`pnpm exec tsc --noEmit`。
...
- [ ] **2.1 检查配置端口**
  ...
  - 验证：Fake 端口观察器测试；`pnpm exec tsc --noEmit`。
...
- [ ] **2.2 用户确认后结束监听进程**
  ...
  - 验证：身份一致、PID 复用、端口变化和保护进程四类测试；`pnpm exec tsc --noEmit`。
...
- [ ] **3.1 将运行实例绑定到现有代理**
  ...
  - 验证：代理绑定、停止失效、端口复用和 Workspace 隔离测试；`pnpm exec tsc --noEmit`。
