...
test('DSH Transport 可通过 Fake Carrier 完成 RPC 和 Fetch', async () => {
  ...
  const rpcFrame = JSON.parse(carrier.sent[0] as string)
  carrier.emit(encodeTunnelFrame({ version: 1, channel: 'rpc', id: rpcFrame.id, sequence: 0, kind: 'data', payload: { ok: true } }))
  assert.deepEqual(await rpc, { ok: true })

  const fetchPromise = transport.fetch('https://example.test/api', { method: 'GET' })
  const fetchFrame = JSON.parse(carrier.sent[1] as string)
  carrier.emit(encodeTunnelFrame({ version: 1, channel: 'fetch', id: fetchFrame.id, sequence: 0, kind: 'data', payload: { status: 200, headers: [['content-type', 'text/plain']], body: 'ok' } }))
...
test('generation 更新会拒绝旧请求并隔离旧响应', async () => {
  ...
  const oldFrame = JSON.parse(carrier.sent[0] as string) as { id: string }
  ...
  const newFrame = JSON.parse(carrier.sent[1] as string) as { id: string }
  assert.match(newFrame.id, /^g2_/u)
  carrier.emit(encodeTunnelFrame({ version: 1, channel: 'rpc', id: oldFrame.id, sequence: 0, kind: 'data', payload: { generation: 1 } }))
  carrier.emit(encodeTunnelFrame({ version: 1, channel: 'rpc', id: newFrame.id, sequence: 0, kind: 'data', payload: { generation: 2 } }))
